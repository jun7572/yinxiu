...
class _CircleProgressWidgetState extends State<CircleProgressWidget> {
  ...
  late ImageInfo imageInfo2;
  initData() async {
    // await Utils.init();
    int size = widget.progress.strokeWidth.toInt();

    imageCircle = await Utils.getImageCircle(imageWidget, imageWidget);
    imageInfo = await Utils.getImageInfo(Get.context!);
    imageInfo2 = await Utils.getImageCircleInfo(Get.context!);
  ...
  bool drawOrNot(Offset touchPoint, Offset smallCircle) {
    ...
    num dis = math.sqrt(math.pow(smallCircle.dx - touchPoint.dx, 2) +
        math.pow(smallCircle.dx - touchPoint.dx, 2));
    print("----------------");
  ...
  setChangeAng(DragUpdateDetails details) {
    ...
    //计算落点手势滑动之后与圆心的角度
    var d = atan2(dx - widget.progress.radius, dy - widget.progress.radius) *
        (180 / pi);
...
//----------------------------------------------j决定起点
  ...
  _drawImgCircle(Canvas canvas) {
    ...
    double sweepAngle =
        _progress.value * 360 + startAng - _changeAng - (startAng + circleAng);
    //半径减去笔触的宽度
    double tempRadius = _progress.radius - 12;
    //小圆的坐标
    double x = tempRadius * cos(sweepAngle / 180 * pi) +
        tempRadius -
        _image.width / 2.5;
    double y = tempRadius * sin(sweepAngle / 180 * pi) +
        tempRadius -
        _image.width / 2.5;
    // var dd = drawOrNot();
    ...
    paintImage(
      ...
      rect: Rect.fromLTWH(
        x,
        y,
        imageInfo2.image.width / 3,
        imageInfo2.image.height / 3,
      ),
      image: imageInfo2.image,
    );

    // canvas.drawCircle(
    //     Offset(x + _image.width / 2, y + _image.width / 2), 4, p1); //查看坐标点
    //偏移中心点坐标,偏移笔触的一半,和图片的一半
    offsetFn(Offset(x + (_image.width / 2) + (_progress.strokeWidth / 2),
        y + (_image.width / 2) + (_progress.strokeWidth / 2)));
  }

  _drawMoon(Canvas canvas) {
    double sweepAngle = startAng;
    //半径减去笔触的宽度
    double tempRadius = _progress.radius - 12;
    //月亮的坐标
    double x = tempRadius * cos(sweepAngle / 180 * pi) +
    ...
    paintImage(
        canvas: canvas,
        rect: Rect.fromLTWH(
            x, y, imageInfo.image.width / 3, imageInfo.image.height / 3),
  ...
  _drawProgress(Canvas canvas) {
    ...
    //改变的角度
    double a1 = (sweepAngle - _changeAng - (startAng + circleAng));
    a1 = (a1 < 360) ? a1 : (a1 % 360);
    //回调进度,这个方法你自己看看回调到哪

    fn(a1 / 360);
    canvas.drawArc(Rect.fromLTRB(0, 0, _radius * 2, _radius * 2),
        startAng / 180 * pi, (a1) / 180 * pi, false, _paint);
    // canvas.restore();
  }
